"""Handler for updating dataset information - fixed version."""
...
class UpdateDatasetHandler:
    ...
    async def handle(self, command: UpdateDatasetCommand) -> UpdateDatasetResponse:
        ...
        if command.name is not None or command.description is not None:
            ...
            update_kwargs = {'dataset_id': command.dataset_id}
            if command.name is not None:
                update_kwargs['name'] = command.name
            if command.description is not None:
                update_kwargs['description'] = command.description
            
            await self._dataset_repo.update_dataset(**update_kwargs)
